...
public class VirtualCameraStereo : MonoBehaviour
{
    ...
    private Material _sideBySideMaterial = null;
    ...
    void Awake()
    {
        ...
        Shader _sideBySideShader = Shader.Find("SideBySideStereo");
        if (_sideBySideShader != null)
        {
            _sideBySideMaterial = new Material(_sideBySideShader);
            _sideBySideMaterial.name = "SideBySideStereo";
        }
        else
        {
            Debug.LogError("Failed to find the SideBySideStereo shader.");
        }
    }
